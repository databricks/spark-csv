...
* `codec`: compression codec to use when saving to file. Should be the fully qualified name of a class implementing `org.apache.hadoop.io.compress.CompressionCodec` or one of case-insensitive shorten names (`none`, `deflate`, `bzip2`, `gzip`, `lz4`, and `snappy`). Defaults to no compression when a codec is not specified.
